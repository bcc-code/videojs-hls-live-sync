var sync = (function (exports) {
    ...
    ***************************************************************************** */
    ...
    var _a$3;
    var ERRORS$1 = (_a$3 = {},
        _a$3["already-exists" /* ALREADY_EXISTS */] = 'A Firebase Analytics instance with the appId {$id} ' +
            ' already exists. ' +
            'Only one Firebase Analytics instance can be created for each appId.',
        _a$3["already-initialized" /* ALREADY_INITIALIZED */] = 'Firebase Analytics has already been initialized.' +
            'settings() must be called before initializing any Analytics instance' +
            'or it will have no effect.',
        _a$3["interop-component-reg-failed" /* INTEROP_COMPONENT_REG_FAILED */] = 'Firebase Analytics Interop Component failed to instantiate: {$reason}',
        _a$3["invalid-analytics-context" /* INVALID_ANALYTICS_CONTEXT */] = 'Firebase Analytics is not supported in this environment. ' +
            'Wrap initialization of analytics in analytics.isSupported() ' +
            'to prevent initialization in unsupported environments. Details: {$errorInfo}',
        _a$3["indexeddb-unavailable" /* INDEXEDDB_UNAVAILABLE */] = 'IndexedDB unavailable or restricted in this environment. ' +
            'Wrap initialization of analytics in analytics.isSupported() ' +
            'to prevent initialization in unsupported environments. Details: {$errorInfo}',
        _a$3["fetch-throttle" /* FETCH_THROTTLE */] = 'The config fetch request timed out while in an exponential backoff state.' +
            ' Unix timestamp in milliseconds when fetch request throttling ends: {$throttleEndTimeMillis}.',
        _a$3["config-fetch-failed" /* CONFIG_FETCH_FAILED */] = 'Dynamic config fetch failed: [{$httpStatus}] {$responseMessage}',
        _a$3["no-api-key" /* NO_API_KEY */] = 'The "apiKey" field is empty in the local Firebase config. Firebase Analytics requires this field to' +
            'contain a valid API key.',
        _a$3["no-app-id" /* NO_APP_ID */] = 'The "appId" field is empty in the local Firebase config. Firebase Analytics requires this field to' +
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 4:
                        _b.sent();
                        return [3 /*break*/, 5];
                    case 5: throw ERROR_FACTORY$2.create("config-fetch-failed" /* CONFIG_FETCH_FAILED */, {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        logger$1.warn(ERROR_FACTORY$2.create("indexeddb-unavailable" /* INDEXEDDB_UNAVAILABLE */, {
    ...
    function warnOnBrowserContextMismatch() {
        ...
        if (mismatchedEnvMessages.length > 0) {
            var details = mismatchedEnvMessages
                ...
                .join(' ');
            var err = ERROR_FACTORY$2.create("invalid-analytics-context" /* INVALID_ANALYTICS_CONTEXT */, {
                errorInfo: details
            });
    ...
    var enableLogging = function (logger_, persistent) {
        ...
        if (logger_ === true) {
            ...
            logger$2 = logClient.log.bind(logClient);
            if (persistent) {
    ...
    var log = function () {
        ...
        if (firstLog_ === true) {
            firstLog_ = false;
            if (logger$2 === null && SessionStorage.get('logging_enabled') === true) {
                enableLogging(true);
            }
        }
        if (logger$2) {
    ...
    async function init(player) {
        ...
        if (document.location.hash == "") {
            ...
            sessionID = document.location.hash.substring(1);
        ...
        (_a = globalPlayer.play()) === null || _a === void 0 ? void 0 : _a.then(playerStarted, console.log);
        (_b = document.getElementById("add")) === null || _b === void 0 ? void 0 : _b.addEventListener("click", add);
    ...
    async function playerStarted() {
        ...
        startTimestamp = now();
        let startTime;
        let segmentRef = firebase$1.database().ref(`${sessionID}/segment`);
        if (master) {
            firebase$1.database().ref(`${sessionID}/start`).set(startTimestamp);
            globalPlayer.currentTime(globalPlayer.liveTracker.liveCurrentTime() - 10);
        }
        else {
            startTime = firebase$1.database().ref(`${sessionID}/start`).get();
            firebase$1.database().ref(`${sessionID}/segment`).on('value', handleSyncData);
        }
        let textTracks = globalPlayer.textTracks();
        let cuesTrack = textTracks[0];
        remoteStart = (_a = (await startTime)) === null || _a === void 0 ? void 0 : _a.val();
